...
Inno Setup: free, script-based (the script is a committed, reviewable file), one `.exe`, per-user without elevation, registers the uninstaller, upgrades in place by `AppId`, and its `[Code]` section holds the already-installed dialog. MSI (WiX) only when a corporate deployment tool demands it — the owner's call at Phase 2. **Inno Setup is a standard machine prerequisite of the bridge** (`winget install --id JRSoftware.InnoSetup -e`; setup guide Step 10 — installed once so any project can build its installer; pin the version in `PROJECT_STATUS.md`); GitHub `windows-latest` runners have it preinstalled. **Locate the compiler, never assume its folder:** a per-user winget install puts it at `%LOCALAPPDATA%\Programs\Inno Setup 6\ISCC.exe`, an all-users install at `%ProgramFiles(x86)%\Inno Setup 6\ISCC.exe`; check both (and `winget list --id JRSoftware.InnoSetup -e` for the version) before writing the build script, and record the resolved path in `PROJECT_STATUS.md`. If neither exists, hand the owner the Step 10 run sheet — do not install it yourself.
